"""
Numerically stable GRPO trainer with enhanced error handling and safety checks
"""
...
class StableGRPOTrainer:
    """
    Numerically stable GRPO trainer with safety checks to prevent inf/nan errors.
    """
    
    def __init__(
        self,
        model_name: str,
        learning_rate: float = 5e-6,  # Lower learning rate for stability
        num_epochs: int = 3,
        batch_size: int = 4,
        k_samples: int = 4,
        temperature: float = 0.8,
        kl_coef: float = 0.2,  # Higher KL penalty for stability
        gamma: float = 0.99,
        device: str = "cuda",
        max_grad_norm: float = 0.5,  # More aggressive gradient clipping
        min_temperature: float = 0.1,  # Never go below this
        numerical_stability_eps: float = 1e-8
    ):
        """
        Initialize stable GRPO trainer with enhanced safety parameters.
        """
        ...
        self.optimizer = torch.optim.AdamW(
            ...
            lr=learning_rate,
            weight_decay=0.01,
            eps=1e-8  # Numerical stability in optimizer
        )
        
        # Learning rate scheduler for stability
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer,
            T_max=num_epochs * 1000,  # Approximate steps
            ...
            with torch.cuda.amp.autocast(enabled=False):
                outputs = self.model.generate(
                    **inputs,
                    ...
                    do_sample=True,
                    top_p=0.95,
                    top_k=50,  # Add top-k for stability
            ...
            if "probability tensor contains either `inf`, `nan`" in str(e):
                ...
                outputs = self.model.generate(
                    **inputs,
                    ...
                    do_sample=True,
                    top_k=10,  # Very restrictive top-k
                    pad_token_id=self.tokenizer.pad_token_id,
                    eos_token_id=self.tokenizer.eos_token_id,
                    return_dict_in_generate=True,
                    output_scores=True
    ...
    def compute_advantages(self, rewards: torch.Tensor) -> torch.Tensor:
        ...
        std = torch.maximum(std, torch.tensor(self.eps).to(self.device))
    ...
    def compute_kl_penalty(
        ...
        if torch.isnan(kl) or torch.isinf(kl):
            ...
            kl = torch.tensor(0.0).to(self.device)
    ...
    def train_step(
        ...
        return {
            'loss': total_loss.item() if torch.is_tensor(total_loss) and not torch.isnan(total_loss).any() else 0.0,
            'reward': rewards.mean().item() if torch.is_tensor(rewards) else 0.0,
            'kl_penalty': losses.mean().item() if torch.is_tensor(losses) else 0.0,
            'advantages': advantages.mean().item() if torch.is_tensor(advantages) else 0.0,
            'gradient_norm': total_norm.item() if 'total_norm' in locals() else 0.0,
    ...
    def train(
        ...
        """
        Full training loop with stability monitoring.
        """
